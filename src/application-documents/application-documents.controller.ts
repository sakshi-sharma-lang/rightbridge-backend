...
export class ApplicationDocumentsController {
  ...
  async getDocumentsForAdmin(@Param('applicationId') applicationId: string) {
    return this.service.getDocumentsForAdmin(applicationId);
  }

  @Get(':applicationId')
  @UseGuards(AuthGuard('jwt'))
  getUserDocuments(@Param('applicationId') applicationId: string, @Req() req) {
    return this.service.getByApplication(applicationId, req.user.userId);
  }
  ...
  async adminDeleteDocument(
    @Body() body: { applicationId: string; type: string; uid: string },
  ) {
