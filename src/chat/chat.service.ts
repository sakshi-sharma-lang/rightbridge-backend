...
async sendMessageByUser(data: any) {
  try {
    ...
    // get exact saved message from DB (last element in messages array)
    const savedSubdoc: any =
      conversation.messages[conversation.messages.length - 1];
    const base =
      savedSubdoc && typeof savedSubdoc.toObject === 'function'
        ? savedSubdoc.toObject()
        : savedSubdoc || {};
 
    // Enrich message with conversation-level metadata so WebSocket
    // receivers can route by conversation/admin/user/application.
    const messageData = {
      ...base,
